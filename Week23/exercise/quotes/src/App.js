...
function App() {
  ...
  const colors = ["#16a085", "#e74c3c", "#9b59b6", "#f39c12", "#2980b9", "#27ae60"];
  ...
  const generateQuote = () => {
    const remainingQuotes = quotes.filter((_, index) => !usedQuotes.includes(index));
    ...
    if (remainingQuotes.length === 0) {
      ...
      const randomQuoteObj = remainingQuotes[Math.floor(Math.random() * remainingQuotes.length)];
      randomIndex = quotes.indexOf(randomQuoteObj);
    }
  ...
  return (
    <div style={{ backgroundColor: currentColor, height: "100vh", transition: "all 0.5s ease" }}>
      <Quotebox 
